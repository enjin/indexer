...
- The processor receives every block, including blocks without subscribed events, so empty replacement blocks can invalidate the old SNS branch. SNS deduplication uses the full block hash and event index within a 30-second in-process cache. Observed block replacements orphan the cached branch; reorganization links require the same signed transaction hash, call address, event name, and occurrence. Unsigned and hook events without unique transaction identity are sent without speculative links. Cache expiry and processor restarts limit replay linking; this is not a durable notification outbox.
