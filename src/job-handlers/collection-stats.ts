...
export default async (job: Queue.Job<JobData>, done: Queue.DoneCallback) => {
    console.log('processing job for collection stats ' + job.data.collectionId)
    ...
    await job.log('Processing collection stats job: ' + job.id)
    console.log('processing job for collection stats ' + collectionId)

    await connection.manager.transaction('READ UNCOMMITTED', async (em) => {
        try {
            const promises = [
                em
                    .createQueryBuilder()
    ...
    console.log('finished processing job for collection stats ' + collectionId)
}
