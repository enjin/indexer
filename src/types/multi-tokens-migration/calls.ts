...
export const migrateCollectionAccounts = {
    ...
    matrixEnjinV603: new CallType(
        ...
        sts.struct({
            accounts: sts.array(() =>
                sts.tuple(() => [sts.bigint(), matrixEnjinV603.AccountId32, matrixEnjinV603.CollectionAccount])
            ),
        })
    ),
}
...
export const migrateTokenAccounts = {
    ...
    matrixEnjinV603: new CallType(
        ...
        sts.struct({
            accounts: sts.array(() =>
                sts.tuple(() => [sts.bigint(), sts.bigint(), matrixEnjinV603.AccountId32, matrixEnjinV603.TokenAccount])
            ),
        })
    ),
}
    ...
    matrixEnjinV603: new CallType(
        ...
        sts.struct({
            attributes: sts.array(() =>
                sts.tuple(() => [sts.bigint(), sts.option(() => sts.bigint()), sts.bytes(), matrixEnjinV603.Attribute])
            ),
        })
    ),
}
