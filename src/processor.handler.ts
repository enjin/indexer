...
import { multiTokens, balances, claims, marketplace, fuelTanks, polkadotXcm, identity } from './type/events'
...
export async function eventHandler(
    ...
    return match(item.name)
        ...
        .with(multiTokens.attributeSet.name, () => p.multiTokens.processors.attributeSet(ctx, block, item, skipSave))
        ...
        .with(multiTokens.tokenCreated.name, () => p.multiTokens.processors.tokenCreated(ctx, block, item, skipSave))
        .with(multiTokens.tokenDestroyed.name, () =>
            p.multiTokens.processors.tokenDestroyed(ctx, block, item, skipSave)
        )
        .with(multiTokens.tokenMutated.name, () => p.multiTokens.processors.tokenMutated(ctx, block, item, skipSave))
        ...
        .with(multiTokens.claimedCollections.name, () => p.multiTokens.processors.claimedCollections(ctx, block, item))
        ...
        .with(marketplace.counterOfferPlaced.name, () => p.marketplace.processors.counterOfferPlaced(ctx, block, item))
        ...
        .with(identity.judgementUnrequested.name, () => p.identity.processors.judgementUnrequested(ctx, block, item))
