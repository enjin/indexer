...
export const upsert = <T>(store: Store, EntityConstructor: EntityConstructor<T>, data: DeepPartial<T>): Promise<T> => {
    const entity = new EntityConstructor(data)
    return store.save(entity)
}

export const insert = async <T>(store: Store, EntityConstructor: EntityConstructor<T>, data: T): Promise<void> => {
...
export const update = async <T>(
    ...
    const updateResult = await store.update(EntityConstructor, criteria, data as QueryDeepPartialEntity<T>)
