...
export class ComputeValidatorsProcessor implements ProcessorDef {
    async handle(job: Job): Promise<void> {
        ...
        for (const validator of validators) {
            try {
                const details = validatorDetails.find((d) => d.id === validator.id)
                if (!details) {
                    continue
                }
                ...
                validator.blockProduction28d = [
                    ...(validator.blockProduction28d ?? []),
                    details.producedBlocks24h,
                ].slice(-28)
...
async function getValidatorDetails(em: EntityManager, ids: string[]): Promise<Record<string, any>[]> {
    return await em
        .getRepository(Validator)
        .createQueryBuilder('validator')
