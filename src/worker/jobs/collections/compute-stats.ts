...
const floorQuery = `SELECT MIN(CASE WHEN COALESCE(("token"."native_metadata"->>'decimalCount')::int, 0) > 0 THEN "listing"."highest_price" * POWER(10, ("token"."native_metadata"->>'decimalCount')::int)::bigint ELSE "listing"."highest_price" END) AS floor_price FROM "listing" AS "listing" INNER JOIN "token" "token" ON ("token"."id" = "listing"."make_asset_id_id" AND "listing"."make_asset_id_id" != '0-0') INNER JOIN "collection" "collection" ON "collection"."id" = "token"."collection_id" WHERE "collection"."id" = $1 AND "listing"."is_active" = TRUE AND "listing"."has_royalty_increased" IS NULL AND "listing"."type" = 'FixedPrice' AND "token"."is_frozen" = FALSE AND "token"."listing_forbidden" = FALSE;`

const activeListingsAmountQuery = `SELECT COALESCE(SUM("listing"."amount"), 0) AS active_listings_amount FROM "listing" AS "listing" INNER JOIN "token" "token" ON ("token"."id" = "listing"."make_asset_id_id" AND "listing"."make_asset_id_id" != '0-0') INNER JOIN "collection" "collection" ON "collection"."id" = "token"."collection_id" WHERE "collection"."id" = $1 AND "listing"."is_active" = TRUE AND "listing"."has_royalty_increased" IS NULL AND ("listing"."type" = 'FixedPrice' OR "listing"."type" = 'Auction') AND ("listing"."state"->>'isExpired' = 'false' OR "listing"."state"->>'isExpired' IS NULL) AND "token"."is_frozen" = FALSE AND "token"."listing_forbidden" = FALSE;`
...
export async function computeStats(_job: Job, collectionId: string) {
    ...
    const promises: Promise<any>[] = [
        ...
        em.query(activeListingsAmountQuery, [collectionId]),
        em
            .getRepository(Token)
