...
import {
    Event,
    StakeExchangeBuyOrderCompleted, StakeExchangeOffer,
    StakeExchangeOfferCancelled,
    StakeExchangeOfferCompleted, StakeExchangeOfferCreated
} from "~/model";
...
export async function syncStakeExchangeOffersEvents(job: Job): Promise<void> {
    ...
    const events: Event[] = await em.find(Event, {
        select: ['data', 'name'],
        where: {
            name: In([StakeExchangeOfferCancelled.name, StakeExchangeOfferCompleted.name, StakeExchangeOfferCreated.name, StakeExchangeBuyOrderCompleted.name])
        }
    })
