...
import { CreateFuelTank } from '~/pallet/fuel-tanks/calls/types'
...
export const createFuelTank = withDispatchCheck((call: CallItem): CreateFuelTank => {
    return match(call)
        ...
        .when(
            () => calls.fuelTanks.createFuelTank.matrixV1040.is(call),
            () => calls.fuelTanks.createFuelTank.matrixV1040.decode(call)
        )
        .when(
            () => calls.fuelTanks.createFuelTank.matrixV1030.is(call),
            () => calls.fuelTanks.createFuelTank.matrixV1030.decode(call)
        )
        .when(
            () => calls.fuelTanks.createFuelTank.matrixV1023.is(call),
            () => calls.fuelTanks.createFuelTank.matrixV1023.decode(call)
        )
        .when(
