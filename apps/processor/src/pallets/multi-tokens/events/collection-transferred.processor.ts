...
import { collectionTransferredMap, CollectionTransferredProcessData } from './collection-transferred.map'

export class CollectionTransferredProcessor extends EventProcessor<
    CollectionTransferred,
    CollectionTransferredProcessData
> {
    constructor() {
        super(multiTokens.collectionTransferred.name, collectionTransferredMap)
    }

    protected decodeEventItem(item: EventItem): CollectionTransferred {
        return collectionTransferredMap.decode(item)
    }
